            ...
            <div className="p-3.5 text-zinc-700">
                ...
                <div className="flex items-center justify-between w-full mt-1">
                    <div className="flex items-baseline gap-1.5 flex-wrap min-w-0">
                        <span className="text-base font-bold text-gray-900">{currency}{product.price.toFixed(1)}</span>
                        <span className="text-xs text-gray-400">/{product.unit}</span>
                    </div>
                    {product.originalPrice > product.price && <span className="text-xs text-gray-400 line-through ml-1.5">{currency}{product.originalPrice.toFixed(1)}</span>}
                </div>
                <button onClick={(e)=>{e.stopPropagation(); addToCart(product)}} className="size-7 flex items-center justify-center rounded-full bg-app-orange text-white shrink-0 shadow-sm hover:opacity-90 transition-opacity">
