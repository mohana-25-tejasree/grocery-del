...
export const inngest = new Inngest({ id: "gorcery-del" });
...
const autoAssignRider = inngest.createFunction({
  ...
  const result = await step.run("assign-rider",async () =>{
    ...
    if(!order) return {skipped:true,reson:"Order not found"};
    if(order.deliveryPartnerId) return {skipped:true,reson:"Already assigned"};
